...
void insereDadaPosicao(TListaPonto *lista, TPonto p);
...
int main(void) {
    ...
    do{
        printf("1 - Adicionar no Final\n"
               "2 - Adicionar na posicao Indicada (Caso vazio, adiciona no final)\n"
...
void insereDadaPosicao(TListaPonto *lista, TPonto p) {
    if (lista->qtdAtual < lista->maximo && lista->qtdAtual > 0) {
        int posicao;
        printf("Digite em qual posicao deseja inserir a Coordenada (***Substitui Coordenada***): ");
        scanf("%d", &posicao);
